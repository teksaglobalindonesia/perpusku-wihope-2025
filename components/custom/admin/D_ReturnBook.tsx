...
export const D_ReturnBooks = ({ statusBookItems = [], pagination }: StatusBukuType) => {
        ...
        const debounceTimer = setTimeout(() => {
            const fetchReturns = async () => {
                const response = await fetch(
                    `${BASE_URL}/api/return/list?page=${page}&page_size=2${searchQuery ? `&search=${encodeURIComponent(searchQuery)}` : ''
                    }`,
                    {
                        method: 'GET',
                        headers: {
            ...
            <div className="mx-auto">
                ...
                <div className="space-y-8">
                    {filteredReturn.length > 0 ? (
                        filteredReturn.map((item: ReturnRecord) => (
                            <div
                                key={item.id}
