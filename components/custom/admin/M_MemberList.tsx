...
export const M_MemberList = ({ userItems = [], maxData = 5 }: UserType) => {
    ...
    const handleEdit = (uid: string) => console.log("EDIT item pada produk:", uid);
    const handlePeminjaman = (uid: string) => console.log("Melihat peminjaman item pada produk:", uid);
    ...
    const confirmDelete = () => {
        if (selectedUser) console.log("Menghapus user:", selectedUser.uid);
        ...
        <div className="flex flex-col mx-10 my-10 border-2 border-gray-200 rounded-lg shadow-md bg-white">
            ...
            {paginatedData.length > 0 ? (
                paginatedData.map((item) => (
                    <div key={item.uid} className="mx-6 my-4">
                        <div className="px-6 py-4 flex justify-between items-center border rounded-md hover:shadow-lg transition-shadow">
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="flex gap-2 mt-2">
                                    <Link href="/dashboard/members/loan"><button onClick={() => handlePeminjaman(item.uid)} className="bg-yellow-300 hover:bg-yellow-400 text-primary-dark px-6 py-2 rounded-md font-medium transition hover:scale-105 shadow-sm hover:shadow-md">PEMINJAMAN</button></Link>
                                    <Link href="/dashboard/members/edit"><button onClick={() => handleEdit(item.uid)} className="bg-yellow-300 hover:bg-yellow-400 text-primary-dark px-6 py-2 rounded-md font-medium transition hover:scale-105 shadow-sm hover:shadow-md">EDIT</button></Link>
                                    <button onClick={() => handleDelete(item.uid, item.name)} className="bg-yellow-300 hover:bg-yellow-400 text-primary-dark px-6 py-2 rounded-md font-medium transition hover:scale-105 shadow-sm hover:shadow-md">HAPUS</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {showDeleteModal && selectedUser && (
                <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
                    <div className="w-[500px] h-[180px] bg-white rounded-xl shadow-lg flex flex-col justify-center items-center gap-4 px-6 py-4">
