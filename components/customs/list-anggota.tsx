        ...
        <div className="w-full bg-[#FFEAC5] mt-16 md:mt-[84px] px-4 md:px-[64px] py-6 md:py-[40px]">
            <div className="w-full px-2 md:px-3 py-3 md:py-5">
                <div className="md:flex md:items-center md:justify-between font-morrisroman">
                    <h1 className="text-2xl md:text-3xl font-semibold">Members</h1>
                    <div className="flex flex-col md:flex-row gap-3 md:gap-4 mt-3 md:mt-0">
                        ...
                        <Link href="/anggota/tambah_member" className="px-4 md:px-8 py-2 clip-custom text-sm md:text-lg font-cyrodiil text-white font-semibold bg-green-500 text-center">
                            Add
                        </Link>
                    </div>
                </div>
                {saatiniMember.map((member) => (
                    <div key={member.id} className="w-full border-2 md:border-4 rounded-md p-3 md:p-4 mt-3 md:mt-5">
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                            <div className="text-sm md:text-lg font-cyrodiil">
                                ...
                                <div className="flex flex-wrap gap-2 md:gap-3 mt-2">
                                    <Link href={`/anggota/pinjaman_member?id=${member.id}`} className="bg-blue-500 px-4 py-1 md:px-8 clip-custom text-xs md:text-base">Borrowing</Link>
                                    <Link href="/anggota/edit_member" className="bg-yellow-400 px-4 py-1 md:px-8 clip-custom text-xs md:text-base">Edit</Link>
        ...
        {munculPopup && (
            <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center bg-[#F2C078] w-[90%] md:w-[500px] h-auto md:h-[142px] p-4 md:p-0 rounded-xl shadow-lg">
                <div className="w-full md:w-64 font-cyrodiil">
                    <h1 className="text-lg md:text-xl text-center md:text-left">Are you sure you want to delete this member?</h1>
                    <div className="flex flex-col sm:flex-row gap-3 md:gap-5 justify-center md:justify-start mt-4 text-base md:text-lg">
                        ...
                        <button className="border-2 rounded-md px-3 py-2 md:px-4 md:py-0" onClick={() => setMunculPopup(false)}>CANCEL</button>
                    </div>
                </div>
            </div>
        )}
