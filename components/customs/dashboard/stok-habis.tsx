            ...
            <div className="w-full px-4 md:px-[64px] py-6">
                ...
                <div className="w-full border-2 md:border-4 rounded-md px-2 md:px-6 py-3 md:py-5">
                    <div className="flex flex-col md:flex-row md:items-center justify-between font-morrisroman gap-2 md:gap-0">
                        ...
                        className="px-4 sm:px-8 border-2 sm:border-4 rounded-md text-sm sm:text-lg w-full sm:w-auto"
                        />
                    </div>

                    <div className="w-full flex flex-col gap-3 md:gap-4 mt-3 md:mt-5">
                        {isSearching && bukuHabis.length === 0 && filterBook.length > 0 ? (
                        <div className="w-full text-center py-10">
                            <p className="text-xl md:text-2xl font-cyrodiil">
                            The book you are looking for is still available
                            </p>
                        </div>
                        ) : bukuHabis.length === 0 ? (
                        <div className="w-full text-center py-10">
                        ...
                        <>
                        {bukuHabis.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((book) => (
                            <div key={book.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-2 md:border-4 rounded-md p-2 md:p-4 gap-2 transition-all duration-300 hover:scale-[1.02] hover:border-[#43766C]">
                                <div className="flex flex-col sm:flex-row gap-3 md:gap-7 items-start sm:items-center w-full">
                                    <div className="relative w-12 h-12 md:w-16 md:h-16 transition-transform duration-300 hover:scale-110">
                                        <Image
                                            src={book.cover ? API + book.cover.url : "/idk"}
                                            alt={book.title}
                                            fill
                                            quality={100}
